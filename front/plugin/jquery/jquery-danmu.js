...
;(function($){
	...
	DanMuMsg.prototype = {
		...
		init: function(fn){
			// 消息初始化
			var _this = this;
			if(_this.conf.msgData.imgpath){// 弹幕显示若头像，进行图片预加载
				// 预加载用户头像
				ImgUtils.loadImg(_this.conf.msgData.imgpath,function(img,e){
					if(e){
						// 加载失败
						_this.conf.msgData.imgpath = defaultHead;
					}
					// 消息初始化
					_this.appendToHtml();
					fn && fn(_this);
	...
	DanMuLine.prototype = {
		checkLine: function(msgData,startFn,finishFn,fn){
			...
			var flag = true;
			// 获取该行最后面的消息剩余运动时间
			// 在配置的最小和最大时间中随机到一个时间，然后判断该时间是否会出现消息重叠
		...
		push: function(msgObj,startFn,finishFn){
			...
			var pushObj = function(msgObj){
				var position = _this.conf.position,
					danMuHeight = _this.conf.danMuJquery.height(),
					lineHeight = _this.conf.lineHeight || $('.line').outerHeight(true),
					lineCount = _this.conf.lineCount || parseInt(danMuHeight / lineHeight),
