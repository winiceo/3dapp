...
!function (document, window, $) {
    $(document).ready(function ($) {
        ...
        for (var length = data.files.length, i = 0; length > i; i++)data.files[i].type.match(/^image\/(gif|jpeg|png|svg\+xml)$/) ? data.files[i].filetype = "image" : data.files[i].filetype = "other-file"
    }).on("fileuploadadded", function (e) {
        var $this = $(e.target);
        $this.find(".file").length > 0 ? $this.addClass("has-file") : $this.removeClass("has-file")
        ...
        0 === $(e.target).parents(".file").length && $("#inputUpload").trigger("click")
    }), $(document).bind("dragover", function (e) {
        var dropZone = $("#exampleUploadForm"), timeout = window.dropZoneTimeout;
        timeout ? clearTimeout(timeout) : dropZone.addClass("in");
        var found = !1, node = e.target;
        do {
            if (node === dropZone[0]) {
                found = !0;
                break
