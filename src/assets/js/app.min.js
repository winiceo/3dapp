    ...
    window.App = Site.extend({
        handleSlidePanel: function() {
            if ("undefined" != typeof $.slidePanel) {
                ...
                $(document).on("click", "[data-toggle=slidePanel]",
                function(e) {
                    $.slidePanel.show({
                        url: $(this).data("url"),
                        settings: {
        ...
        handleMultiSelect: function() {
            var $all = $(".select-all");
            $(document).on("change", ".multi-select",
            function(e, isSelectAll) {
                if (!isSelectAll) {
                    var $select = $(".multi-select"),
                    total = $select.length,
                    checked = $select.find("input:checked").length;
                    total === checked ? $all.find("input").prop("checked", !0) : $all.find("input").prop("checked", !1)
                }
            }),
            $all.on("change",
            function() {
                ...
                $(".multi-select input").each(function() {
                    $(this).prop("checked", checked).trigger("change", [!0])
                })
            })
        },
        handleListActions: function() {
            $(document).on("click", '[data-toggle="list-editable"]',
            function() {
                var $btn = $(this),
            ...
            $(document).on("keydown", ".list-editable [data-bind]",
            function(event) {
                var keycode = event.keyCode ? event.keyCode: event.which;
                if (13 == keycode || 27 == keycode) {
                    var $input = $(this),
                    ...
                    13 == keycode ? $update.html($input.val()) : $input.val($update.text()),
                    $content.show(),
                    $editable.hide()
                }
            }),
            $(document).on("click", '[data-toggle="list-editable-close"]',
            function() {
                var $btn = $(this),
                $list = $btn.parents(".list-group-item"),
                $content = $list.find(".list-content"),
