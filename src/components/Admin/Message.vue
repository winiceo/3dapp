    ...
    <div class="page animsition">
        ...
        <div class="page-main">
            ...
            <div class="app-message-chats">
                <button type="button" id="historyBtn" class="btn btn-round btn-outline btn-default">History Messages
                </button>
                <div class="chats">
                    <div class="chat">
...
<script>
    ...
    require('../assets/examples/css/apps/message.min.css');
                ...
                window.AppMessage = App.extend({
                    scrollChatsToBottom: function () {
                        var $chatsWrap = $(".app-message-chats"),
                                chatsWrapH = $chatsWrap.height(),
                                chatsH = $(".chats", $chatsWrap).outerHeight(),
                                historyBtnH = $("#historyBtn").outerHeight();
                        ...
                        var self = this, $chatsWrap = $(".app-message-chats"), $textarea = $(".message-input textarea"), $textareaWrap = $(".app-message-input");
                        autosize($(".message-input textarea")),
                                $textarea.on("autosize:resized", function () {
                                    var height = $textareaWrap.outerHeight();

                                    $chatsWrap.css("height", "calc(100% - " + height + "px)"), self.scrollChatsToBottom()
                                }),
                                $(".message-input-btn>button").on("click", function () {
                                    var talkContents = $(".message-input>.form-control").val(), $newMsg = $("<div class='chat-content'><p>" + talkContents + "</p></div>");
                                    talkContents.length > 0 ? ($(".chat").last().find(".chat-body").append($newMsg), $(".message-input>.form-control").attr("placeholder", ""), $(".message-input>.form-control").val("")) : $(".message-input>.form-control").attr("placeholder", "type text here..."), $(".message-input>.form-control").focus(), self.scrollChatsToBottom()
                                })
                        var height = $textareaWrap.outerHeight();

                    }, run: function (next) {
                        this.scrollChatsToBottom(), this.handleResize(), this.handleTalking()
                    }
